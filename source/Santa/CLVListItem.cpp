...
BRect CLVListItem::ItemColumnFrame(int32 column_index, const ColumnListView* owner) const
{
	const BList* ColumnList = &owner->fColumnList;
	CLVColumn* ThisColumn = (CLVColumn*)ColumnList->ItemAt(column_index);
	if(!ThisColumn->IsShown())
		return BRect(-1,-1,-1,-1);

	int32 DisplayIndex = owner->IndexOf((BListItem*)this);
	BRect itemRect;
	if(DisplayIndex >= 0)
		itemRect = ((ColumnListView*)owner)->ItemFrame(DisplayIndex);
	else
		return BRect(-1,-1,-1,-1);
	...
	{
		const BList* DisplayList = &owner->fColumnDisplayList;
...
float CLVListItem::ExpanderShift(int32 column_index, const ColumnListView* owner) const
{
...
bool CLVListItem::Editable(int32 column_index) const
{
	return false;
}


const char* CLVListItem::EditInitialText(int32 column_index) const
{
	return NULL;
}


BPoint CLVListItem::ColumnTextDrawPoint(int32 column_index, const ColumnListView* owner) const
{
	return BPoint(0,0);
}


void CLVListItem::GetEditingFont(int32 column_index, BFont* font, const ColumnListView* owner)
	const
{
	return owner->GetFont(font);
}


void CLVListItem::EditCompleted(int32 column_index, const char* text, ColumnListView* owner)
{ }
