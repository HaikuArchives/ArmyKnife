...
class ColumnListView : public BListView
{
	public:
		...
		ColumnListView(	BRect Frame,
						...
						uint32 flags = B_WILL_DRAW | B_FRAME_EVENTS | B_NAVIGABLE | B_PULSE_NEEDED,
		...
		int32 ColumnIndex(int32 item_index, float x);
		...
		virtual void EditCompleted(const char* text);
		virtual void EditAborted();

		void ShowAsInactiveWhenNotKeyboardFocus();
	...
	private:
		...
		bool fSoleSelectionAtRelease;
		bool fWatchingForEdit;
		BRect fEditWatchRect;
		bigtime_t fStartEditWatchTime;
		CLVListItem* fEditItem;
		CLVColumn* fEditColumn;
		CLVEditTextView* fEditTextView;
		rgb_color fSelectedItemColorWindowActive;
		rgb_color fSelectedItemColorWindowInactive;
		bool fWindowActive;
		bool fShowAsInactiveWhenNotKeyboardFocus;
};
...
class CLVEditTextView : public BTextView
{
	public:
		CLVEditTextView(BRect text_rect, const char* edit_text, rgb_color background);
		virtual ~CLVEditTextView();

		virtual void AttachedToWindow();
		virtual void MakeFocus(bool focus);
		virtual void WindowActivated(bool active);
		virtual void KeyDown(const char* bytes, int32 num_bytes);
		void DoneEditing(bool ok_to_activate = true);
			//Removes the CLVEditTextView from its parent, calls the parent's EditCompleted method if
			//the text has changed, and deletes itself.
