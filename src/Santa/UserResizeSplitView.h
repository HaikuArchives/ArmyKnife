...
//**** UserResizeSplitView
//******************************************************************************************************
class UserResizeSplitView : public BView
{
	public:
		UserResizeSplitView(BRect frame, const char* name, float divider_left_or_top,
			orientation posture = B_HORIZONTAL, bool should_resize_left_or_top = true,
			bool should_resize_right_or_bottom = true, bool move_slider_on_frame_resize = false,
			uint32 resize_mask = B_FOLLOW_LEFT|B_FOLLOW_TOP, uint32 flags = B_WILL_DRAW|B_FRAME_EVENTS);
		...
		void AddChildren(BView* top_or_left, BView* right_or_bottom);
		inline BView* LeftChild() {assert(m_posture == B_HORIZONTAL); return m_left_or_top;}
		inline BView* TopChild() {assert(m_posture == B_VERTICAL); return m_left_or_top;}
		inline BView* RightChild() {assert(m_posture == B_HORIZONTAL); return m_right_or_bottom;}
		inline BView* BottomChild() {assert(m_posture == B_VERTICAL); return m_right_or_bottom;}
		...
		void ResizeTo(float width, float height);	//Note: not virtual!  Using this on the BView will
													//interfere with the splitter position.
	private:
		...
		bool m_dragging;
		BView* m_left_or_top;
		BView* m_right_or_bottom;
		float m_left_or_top_minimum_size;
		float m_right_or_bottom_minimum_size;
